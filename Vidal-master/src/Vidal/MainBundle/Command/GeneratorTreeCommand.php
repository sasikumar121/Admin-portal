...
class GeneratorTreeCommand extends ContainerAwareCommand
{
	protected function configure()
	{
		$this->setName('vidal:generator_tree');
	}

	protected function execute(InputInterface $input, OutputInterface $output)
	{
		ini_set('memory_limit', -1);
		$output->writeln('--- vidal:generator_tree started');
		...
		$phpWord->addFontStyle(
			$italicStyle,
			array('name' => 'Tahoma', 'size' => 10, 'color' => '1B2232', 'italic' => true)
		);
		...
		foreach ($grouped as $rTitle => $items) {
			...
			foreach ($items['types'] as $tTitle => $typeItems) {
				...
				$e = $section->addText('   Подраздел: ' . $tTitle, $boldStyle);

				foreach ($typeItems['categories'] as $cTitle => $categoryItems) {
					$text[] = '';
					$text[] = '... ' . mb_strtoupper($cTitle, 'UTF-8');
					$e = $section->addText('      Категория: ' . $cTitle, $italicStyle);
				}
			}
		}

		$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
		$objWriter->save($webRoot . '/arts_tree.docx');
		...
		$output->writeln('+++ vidal:generator_tree completed!');
	}
}